...
if ( ! function_exists( 'wpteam_scripts' ) ) :
    function wpteam_scripts() {
        ...
        wp_enqueue_style( 'mailchimp-stylesheet', get_template_directory_uri() . '/styles/mailchimp.css', array());

        $in_footer = apply_filters('wpteam_load_jquery_in_footer', false);
        wp_deregister_script( 'jquery' );
        wp_enqueue_script( 'jquery', '//ajax.googleapis.com/ajax/libs/jquery/2.1.0/jquery.min.js', array(), '2.1.0', $in_footer );

        wp_enqueue_script( 'main-scripts', get_template_directory_uri() . '/scripts/scripts.js', array('jquery'), $js_version, false );
    }
